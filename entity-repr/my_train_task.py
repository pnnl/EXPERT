...
def parse_args():
    ...
    parser.add_argument(
        "--tuned_model_dir", type=str, default=None, help="tuned model path where we want to load the tuned parameters"
    )
    parser.add_argument('--freeze_bert', action='store_true', help="Whether to freeze BERT")
...
def main():
    ...
    # Loading model
    config.num_labels = len(enttype2idx)
    ...
    if args.freeze_bert: # freeze BERT if needed
        frozen = ['bert.embeddings', 'bert.encoder'] # change if bert is not the LM used
    else:
        frozen = []
    ...
    logger.info(f"num of optimizing parameters: {sum([sum([torch.numel(_p) for _p in _g['params']]) for _g in optimizer_grouped_parameters])}")
    ...
    # ####
    # # Evaluate properties
    # evaluate_properties(model, args, accelerator, eval_align_dataloader, eval_uniform_dataloader)
    # ####

    # Scheduler and math around the number of training steps.
    args.actual_train_step_len = min(len(train_dataloader), args.train_data_cap) # subject to change
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            if step >= args.train_data_cap:
                break
            input_ids_without_masks, input_ids_with_masks, attention_ids, recon_labels, entity_type = batch
            batch = {'input_ids': input_ids_without_masks, \
                'attention_mask': attention_ids, \
                'labels': entity_type, \
                }
            assert len(batch['input_ids']) == 1 # can remove for task training
            outputs = model(**batch) # NEED DEBUG: want a classifier over the entity word representations
            loss = outputs.loss
        ...
        # # evaluate on properties # NEED DEBUG
